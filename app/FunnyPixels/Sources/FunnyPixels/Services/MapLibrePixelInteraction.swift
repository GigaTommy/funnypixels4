...
public class MapLibrePixelClickDetector {
    ...
    public init(
        mapView: MLNMapView,
        layerIdentifiers: [String] = ["pixels-circles", "pixels-squares"]
    ) {
...
public class MapLibrePixelDrawingManager: ObservableObject {
    ...
    /// 当前选中的颜色
    public var selectedColor: String = "#FF6B6B"
    ...
    private func showTempPixel(at coordinate: CLLocationCoordinate2D) {
        ...
        feature.attributes = ["color": UIColor(hexString: selectedColor) ?? UIColor.red]
        ...
        circleLayer.circleColor = NSExpression(forConstantValue: UIColor(hexString: selectedColor) ?? UIColor.red)
...
public class MapLibreGestureHandler: NSObject {
    ...
    /// 绘制像素回调
    private var onPixelDraw: ((CLLocationCoordinate2D, String) -> Void)?
    ...
    /// 设置绘制像素回调
    public func setPixelDrawHandler(_ handler: @escaping (CLLocationCoordinate2D, String) -> Void) {
        onPixelDraw = handler
    }
    ...
    public func handleTap(at point: CGPoint) {
        ...
        if drawingManager.isDrawingMode {
            ...
            if coordinate.latitude != 0 && coordinate.longitude != 0 {
                onPixelDraw?(coordinate, drawingManager.selectedColor)
            }
            return
        }
...
public class MapLibreInteractionManager: NSObject {
    ...
    /// 设置绘制像素回调
    public func onPixelDraw(_ handler: @escaping (CLLocationCoordinate2D, String) -> Void) {
        gestureHandler?.setPixelDrawHandler(handler)
    }

    /// 切换绘制模式
    public func toggleDrawingMode() {
        drawingManager.isDrawingMode.toggle()
