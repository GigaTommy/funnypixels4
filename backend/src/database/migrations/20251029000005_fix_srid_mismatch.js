/**
 * 修复SRID不匹配问题
 * OSM数据使用SRID 3857，需要将查询点转换为相同SRID
 */

exports.up = async function(knex) {
  try {
    console.log('🔧 修复PostGIS函数的SRID不匹配问题...');
    ...
    await knex.raw(`
      ...
      BEGIN
          ...
          SELECT
              ...
              ST_Distance(c.way, (SELECT geom FROM pt))::FLOAT as distance_m,
              CASE
    ...
    console.log('✅ PostGIS函数SRID不匹配问题修复完成');

  } catch (error) {
    console.error('❌ PostGIS函数SRID修复失败:', error.message);
    throw error;
  }
};
...
exports.down = async function(knex) {
  try {
    console.log('🔄 回滚PostGIS函数SRID修复...');

    await knex.raw('DROP FUNCTION IF EXISTS match_point_to_admin_contains(FLOAT, FLOAT)');
    await knex.raw('DROP FUNCTION IF EXISTS match_point_to_admin_distance(FLOAT, FLOAT, INTEGER)');
    await knex.raw('DROP FUNCTION IF EXISTS match_point_to_admin_smart(FLOAT, FLOAT, INTEGER)');

    console.log('✅ PostGIS函数SRID修复回滚完成');

  } catch (error) {
    console.error('❌ PostGIS函数SRID修复回滚失败:', error.message);
    throw error;
  }
};