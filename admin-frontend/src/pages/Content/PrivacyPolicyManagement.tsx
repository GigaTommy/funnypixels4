...
const PrivacyPolicyManagement: React.FC = () => {
  ...
  // 根据语言生成 key
  const getBaseKey = useCallback((lang: string) => `privacy_policy_${lang}`, [])
  const getDraftKey = useCallback((lang: string) => `privacy_policy_${lang}_draft`, [])
    ...
    try {
      ...
      if (editMode === 'file') {
        ...
        if (fileList.length > 0) {
          ...
          formData.append('description', '隐私保护政策(草稿)')
  ...
  const handlePreview = (isDraft: boolean) => {
    if (editMode === 'text' || (isDraft ? draftConfig?.config_type : publishedConfig?.config_type) !== 'file') {
      // Markdown/Text 模式
      const url = `/api/system-config/public/privacy-policy?lang=${currentLang}${isDraft ? '&preview=draft' : ''}`
      ...
      >
        <Spin spinning={loading}>
          ...
          >
            ...
            >
              <Input placeholder="例如：按照新规更新隐私条款" maxLength={200} />
            </Form.Item>
