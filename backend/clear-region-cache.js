 ...
 * 清空地区排行榜缓存
 *
 * 当排行榜 API 数据结构变更后，需要清空缓存以返回新格式数据
 */

const redis = require('redis');

async function clearRegionLeaderboardCache() {
  let client = null;
  try {
    console.log('🧹 开始清空地区排行榜缓存...');
    ...
    // 获取所有地区排行榜相关的键
    const keys = await client.keys('region_leaderboard:*');
    const regionKeys = await client.keys('region:leaderboard:*');
    const allKeys = [...keys, ...regionKeys];

    if (allKeys.length === 0) {
      console.log('✅ 没有找到地区排行榜缓存');
      await client.quit();
      process.exit(0);
    }
